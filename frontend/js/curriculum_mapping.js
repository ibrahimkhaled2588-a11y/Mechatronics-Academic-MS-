...
// This calls the indicators API, which still requires being logged in
// (that's the core indicators-tracker auth, unrelated to this page). This
// page itself has no login requirement, so a signed-out visitor just sees
// a plain message here instead of an error -- everything else on the page
// (ILOs, courses, matrix, findings) works with no login at all.
...
async function loadStandard2Indicators() {
    ...
    try {
        ...
        container.innerHTML = `<p class="section-desc">${t('curr.std2LoginHint')}</p>`;
        return;
    }
