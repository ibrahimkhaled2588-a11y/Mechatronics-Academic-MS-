...
- `backend/auth.py` – Team login for the whole site: 7 member accounts (one per standard) plus an admin account. The root path `/` serves the login page directly. A member can only see/edit indicators under their own standard (the `/api/indicators*` routes enforce this server-side — filtered lists, 404 instead of 403 on other standards' indicator IDs to avoid confirming they exist) and is redirected away from every other page (`frontend/js/page_guard.js`) back to the indicators tracker. The admin sees/edits everything and manages accounts via a "Manage Team Access" panel or `backend/scripts/seed_team_accounts.py`. Stdlib PBKDF2 password hashing + server-side session tokens in an httpOnly cookie — no new dependencies. **Not yet covered**: the other modules' own APIs (governance, faculty, resources, curriculum, alumni, uploads) have no auth check of their own — only the page-level redirect and the indicators API are actually access-controlled.  
